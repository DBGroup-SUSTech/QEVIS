...
export default {
    name: "Task",
    props: ['app', 'taskList', 'status', 'id', 'ch', 'width', 'height'],
    data() {
        let ch = 30
    ...
    watch: {
        ...
        renderSign() {
            ...
            this.cpuLines = atomCPULine(this.status, [this.countHeight, 0], this.xScale);
    ...
    computed: {
        size(){
            return Math.max(0, this.width - 2 * this.gap);
        },
        containerTransform() {
